...
class SpotifyService:
    ...
    @staticmethod
    def import_song_from_spotify(db: Session, query: str) -> Optional[Song]:
        """
        Search for a track on Spotify by query, map it into our Song schema,
        store it in the database, and return the created Song.
        """
        ...
        created_song = SongService.create_song(db, song_create)
        logger.info(f"Song imported successfully: {created_song.id} - {title}")
        return created_song

    @staticmethod
    def import_songs_from_spotify(db: Session, query: str, limit: int = 20) -> List[Song]:
        """
        Search for multiple tracks on Spotify by query, map them into our Song schema,
        store them in the database, and return the created Songs.
        """
