...
def create_song(song: SongCreate, db: Session = Depends(get_db), payload=Depends(verify_token)):
    return SongService.create_song(db, song)
...
def get_songs(
    ...
    return SongService.get_all_songs(db, skip, limit)


@router.get("/{song_id}", response_model=SongResponse)
def get_song(song_id: int, db: Session = Depends(get_db)):
    return SongService.get_song(db, song_id)
...
def update_song(
    ...
    return SongService.update_song(db, song_id, song_update)
...
def search_songs(
    ...
    songs = SongService.search_songs(db, q, skip, limit)
...
def import_song_from_spotify(
    ...
    and return the created Song.
    """
    created_song = SpotifyService.import_song_from_spotify(db, query)
    if not created_song:
        raise HTTPException(
...
def import_songs_from_spotify(
    ...
    """
    try:
        created_songs = SpotifyService.import_songs_from_spotify(db, query, limit)
